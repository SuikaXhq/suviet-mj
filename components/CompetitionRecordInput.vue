...
<template>
    <div class="flex flex-col w-full">
        ...
        <form class="px-2 w-full flex flex-row items-center justify-center gap-2"
            ...
            <div v-for="(player, index) in players" :key="index"
                ...
                <div class="flex flex-col items-center justify-center gap-2">
                    ...
                    <input v-model="points[index]" type="number" step="0.1" placeholder="精算" required
                        class="sm:w-[70px] w-12 sm:h-8 h-4 sm:text-xl text-sm text-right sm:rounded-lg rounded-md"
                        @update="$emit('update')" />
                </div>
            ...
            <button type="submit" class="h-full" :disabled="isSubmitting">
                <BaseButton class="h-full w-16 p-4 flex items-center justify-center" :disabled="isSubmitting">
